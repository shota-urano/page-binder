...
2. SAF ピッカーは前回選択した Drive 内の場所を保持する。また、My Drive にファイルが蓄積すると Provider の一覧描画完了までの時間が変動する。このため自動テストは、初期位置を前提にせず毎回 roots drawer を開いて `Drive` → `My Drive` と選択し直し、画面遷移の idle 待ちと最大60秒のノード再試行を行う。起動直後に roots ボタンを取得できない場合は、現在のピッカーを取消して1回だけ再起動する。取消結果は DocumentsUI の画面が閉じて表示が安定するまで待ってから次のピッカーを起動する。結果通知直後に次を起動すると、前のピッカーの終了処理が新しいピッカーと競合し得るためである。取消操作の Back は現在位置によって「上位階層へ移動」と「ピッカー取消」の2段階になる。
3. Provider URI からの読み戻し成功は確認できたが、Drive サーバーへの同期完了時刻を SAF API から確定することはできない。
4. 負荷の高いエミュレータでは system_server が「System UI isn't responding」等の AppErrorDialog（package `android` / `android:id/aerr_*`）をピッカーの上へ出す。このダイアログはモーダルのため、アクセシビリティのウィンドウ一覧から DocumentsUI のウィンドウごと消える。したがって「アクティブウィンドウが DocumentsUI でない＝ピッカーが閉じた」と判定してはならない。テストは各ポーリングでこのダイアログを（対象プロセスを生かす `aerr_wait` を優先して）自分で閉じ、ピッカーの消滅は DocumentsUI ウィンドウの不在が安定して続くことで判定する（2026-09-02、`pagebinder-x2o`）。
5. 実 UI 操作テストは前回の実行が残した画面状態を引き継いではならない。本テストは `@Before` で DocumentsUI を `am force-stop` し、HOME へ戻し、システムエラーダイアログを閉じ、ピッカー結果キューを空にしてから、その前提が成立していることを assert する。ピッカー起動の再試行ごとにも同じリセットを行う。
