 ...
 *
 * [canStartExport] が true のときだけ書き出しを開始してよい、というのがこの部品の唯一の規則。
 * 判定を実際に効かせるのは画面単位の [ExportViewModel]（書き出し開始の唯一の入口でここを見る）。
...
data class ExportConsentUiState(
    ...
    /** チェックの切り替え。チェックが付いた時点で未確認の案内は消す */
    fun withPermissionConfirmed(confirmed: Boolean): ExportConsentUiState =
        copy(
            permissionConfirmed = confirmed,
            confirmationRequiredVisible = if (confirmed) false else confirmationRequiredVisible,
        )

    /** 未確認のまま書き出しを要求されたときの状態 */
    fun withConfirmationRequired(): ExportConsentUiState = copy(confirmationRequiredVisible = true)
}
